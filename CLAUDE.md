...
| `AGENT_H/interrupt_verifier.py` | `InterruptVerifier` + `PLICModel` — interrupt-controller checker: PLIC priority arbitration (claim = highest pending+enabled source above threshold, tie→lowest id, prio 0 = never; threshold/priority0 masking; claim clears pending) + CLINT (mtip iff mtime≥mtimecmp, msip). Golden over an `interrupt_trace.jsonl` |
