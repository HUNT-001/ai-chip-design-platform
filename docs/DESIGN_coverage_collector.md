...
| **cross (opcode × result value-class)** | `cross:{mnem}:{valclass}` | 12 arith × 6 = 72 | 2 |
| **operand cross (srcA × srcB class)** | `opnd:{class}:{class}` | 6 × 6 = 36 | 2 |
...
**Cross-coverage** (`cross:add:neg`, `cross:sub:all_ones`, …) is a high
...
(`cross_instructions`, model-overridable).

**Operand cross-coverage** (`opnd:neg:zero`, `opnd:all_ones:all_ones`, …) goes
one step deeper — the *input* corner cases. It requires the source-operand
values, which the commit log doesn't record directly, so the collector keeps a
**golden register-file shadow**: it recovers each binary op's source values from
the shadow *before* applying that instruction's write (so `add x5,x5,x6` reads
the old `x5`, not the result), resolves register and immediate operands, and
bins the `(classA, classB)` pair. Both cross categories have matching stimulus
templates, so the self-evolving loop closes them too.
...
- **Temporal cross** — instruction-pair / sequence coverage (e.g. load→use,
  branch→branch) for deeper pipeline corner cases.
