...
# ═══════════════════════════════════════════════════════════════════════════════
# Utility functions — imported by coverage_pipeline.py in manifest mode
# These are the canonical implementations; coverage_pipeline.py re-exports them.
# ═══════════════════════════════════════════════════════════════════════════════
...
def format_ava_schema(
    ...
    *,
    ...
    """
    Format a coverage metrics dict into the canonical AVA inter-agent schema v3.0.

    This matches ``schemas/run_manifest.schema.json`` and is the single source
    of truth for the ``coveragesummary.json`` output contract.

    Parameters
    ----------
    ava_dict         : {"line": float, "branch": float, ...} from CoverageMetrics
    raw_counts       : per-metric hit/total counts
    industrial_grade : True when all primary metrics clear industry thresholds
    run_id           : simulation run identifier
    generated_at     : ISO-8601 timestamp string
    source_file      : path to the .dat file that was parsed
    """
    from datetime import datetime, timezone

    total_hit = sum(v.get("hit",   0) for v in (raw_counts or {}).values())
    total_pts = sum(v.get("total", 0) for v in (raw_counts or {}).values())
    func_pct  = round(ava_dict.get("functional", 0.0), 2)
    ...
    return {
        ...
        # These are filled by the caller after DB / ranker queries:
        "plateau_detected":False,
        "top_cold_paths":  [],
        "cold_paths":      {"lines": [], "branches": [], "toggles": []},
    }
...
def atomic_write(path: "Path", content: str, encoding: str = "utf-8") -> None:
    ...
    Uses a unique per-call temp file (<path>.<pid>.<tid>.tmp) so concurrent
    writers on the same path never overwrite each other's in-progress file.
    POSIX: Path.replace() is atomic.  Windows: atomic since Python 3.3.
    """
    import contextlib as _ctx, os as _os, threading as _th
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    unique = f"{_os.getpid()}.{_th.get_ident()}"
