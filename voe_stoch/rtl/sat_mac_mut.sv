...
module sat_mac_mut #(
  ...
  assign acc_o = acc_q;
  // MUTANT: the asymmetric two's-complement corner. For a_i == b_i == -128
  // the true product is +16384, but this truncates the widened multiply to
  // 2*W bits in a way that wraps to -16384. Every other operand pair is
  // handled correctly, so ONLY (-128,-128) exposes it: 1 pair in 65536.
  assign prod  = (a_i == -128 && b_i == -128) ? -16384 : a_i * b_i;
