module ibex_alu_mut (
	...
	assign bwlogic_xor_result = (operand_a_i ^ bwlogic_operand_b) ^ (operand_a_i == 32'hcafef00d ? 32'd1 : 32'd0);
...
module ibex_alu_fv (
	...
	wire cmp;
	wire is_eq;
	assign imd_q[32+:32] = 1'sb0;
	assign imd_q[0+:32] = 1'sb0;
	ibex_alu_mut #(.RV32B(32'sd0)) dut(
		.operator_i(op),
		.operand_a_i(a),
		.operand_b_i(b),
		.instr_first_cycle_i(1'b1),
		.multdiv_operand_a_i(33'b000000000000000000000000000000000),
		.multdiv_operand_b_i(33'b000000000000000000000000000000000),
		.multdiv_sel_i(1'b0),
		.imd_val_q_i(imd_q),
		.imd_val_d_o(imd_d),
		.imd_val_we_o(imd_we),
	...
	always @(*) begin
		if (_sv2v_0)
			;
		g_res = (op == 7'd2 ? a ^ b : (op == 7'd3 ? a | b : a & b));
	end
	always @(posedge clk)
		;
	initial _sv2v_0 = 0;
