...
import sys as _sys
_SIM_DIR = Path(__file__).resolve().parent / "AGENT_C"  # Agent C modules live here
...
class SpikeISS:
    ...
    async def _compare_results(
        ...
        for seq in range(n_compared):
            ...
            # Register writes
            rtl_regs = {rw["rd"]: rw["value"] for rw in rtl.get("reg_writes", [])}
            iss_regs = {rw["rd"]: rw["value"] for rw in iss.get("reg_writes", [])}
            ...
            # CSR writes
            rtl_csrs = {cw["addr"]: cw["value"] for cw in rtl.get("csr_writes", [])}
            iss_csrs = {cw["addr"]: cw["value"] for cw in iss.get("csr_writes", [])}
